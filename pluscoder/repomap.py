...
def analyze_file_with_tree_sitter(file_path: str, level: int, io) -> str:
    ...
    def process_node(node, indent=''):
        if node.type in ['class_definition', 'class_declaration']  + ['function_definition', 'method_definition', 'function_declaration'] + ['expression_statement']:
            ...
            if level >= 1:
                ...
                for child in node.children:
                    if child.type in ['block', 'statement_block', 'class_body']:
                        ...
                        # Check inner methods
                        for grandchild in child.children:
                            if level >= 2 and grandchild.type in ['function_definition', 'method_definition']:
